    ...
    public void sendWithData(ReadableMap map){
        if( mChannelCore != null ){
            ...
            while( iter.hasNextKey() ){
                ...
                try {
                    if(ReadableType.Map == map.getType(key) ){
                        data.put(key, map.getMap(key));
                    } else if ( ReadableType.Array == map.getType(key) ){
                        data.put(key, map.getArray(key));
                    } else if ( ReadableType.Boolean == map.getType(key) ){
                        data.put(key, map.getBoolean(key));
                    } else if ( ReadableType.Array == map.getType(key) ){
                        data.put(key, map.getArray(key));
                    } else if ( ReadableType.Number == map.getType(key) ) {
                        try {
                            data.put(key, map.getInt(key));
                        } catch( NumberFormatException ne ) {
                            data.put(key, map.getDouble(key));
                        }
